...
def main():
    arboles = divergencia_simple(1, 2_000)
    print(arboles.draw_text())

    arboles = mutacion_simple(arboles)
    genotipos = arboles.genotype_matrix()
    print(f'Genotipos:\n{genotipos}')

    arboles   = divergencia_simple(600, 50_000)
    arboles   = mutacion_simple(arboles)
    genotipos = arboles.genotype_matrix()
    guarda_genotipos(genotipos, 3, 500, 100, 1000)
